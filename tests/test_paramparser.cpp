...
TEST(TestParamParser, MultilineParamNameError)
{
    assert_exception<figcone::ConfigError>([&] {
        ...
        EXPECT_EQ(std::string{error.what()}, "[line:1, column:6] Wrong param 'test' format: parameter's value must be placed on the same line as its name");
    });
}
...
TEST(TestParamParser, MultilineParamError)
{
    assert_exception<figcone::ConfigError>([&] {
        ...
        EXPECT_EQ(std::string{error.what()}, "[line:1, column:8] Wrong param 'test' format: "
